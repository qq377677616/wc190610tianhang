...
vueTouch.prototype = {
  start: function(e) {//监听touchstart事件
    ...
    if (Math.abs(disX) > 10 || Math.abs(disY) > 50) {//当横向位移大于10，纵向位移大于100，则判定为滑动事件
    ...
    this.touchType == "drag" && this.vueCallBack({drag: true});
  },
  move: function(e) {//监听touchmove事件
    if (this.touchType == "drag") {
      var disX = e.changedTouches[0].pageX - this.vueTouches.x;
      var disY = e.changedTouches[0].pageY - this.vueTouches.y;
      this.vueCallBack({x: disX, y: disY});
    }
    this.vueMoves = false;
  }
};
...
Vue.directive("drag",{//拖拽
  bind: function(el,binding){
    new vueTouch(el,binding,"drag");
  }
});
